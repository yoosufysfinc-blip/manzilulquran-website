"use strict";
/* Academic Service — ws-individual.v1.js (split from index.html). Immutable: edit a copy as v2. */
/* ==========================================================================
...
Pages.plans = function(){
  ...
  const rows = DataService.getPlans().map(function(p){
    ...
    return Object.assign({}, p, { studentName: st ? st.name : p.studentId, teacherName: t ? t.name : "—",
      cs: cs, thisMonth: fee ? fee.netFee : Logic.computeGross(p.feeType, p.rate, cs) - (+p.discount || 0),
  ...
  '<div class="card"><div class="card-bd tight">' +
    renderList("plans", rows, {
      ...
      detail: p => dl([
        ...
        ["Fee method", esc(Logic.methodLabel(p.feeType))], ["Rate", money(p.rate)],
        ...
        [monthShort(m) + " fee", money(p.thisMonth)],
