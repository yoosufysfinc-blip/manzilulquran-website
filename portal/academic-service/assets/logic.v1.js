"use strict";
/* Academic Service — logic.v1.js (split from index.html). Immutable: edit a copy as v2. */
/* ==========================================================================
   LOGIC
   ========================================================================== */
const FEE_METHODS = [
...
const Logic = {
  ...
  feeView(f){
    ...
    const late = (status === "Paid" || status === "Waived" || status === "Nil") ? 0 : Math.max(0, diffDays(f.dueDate, t));
  ...
  summarize(views){
    const s = { count: views.length, gross: 0, discount: 0, carry: 0, net: 0, paid: 0, balance: 0, overdueAmt: 0,
      paidN: 0, partialN: 0, overdueN: 0, upcomingN: 0, dueTodayN: 0, waivedN: 0, students: {} };
  ...
  buildBatchMonth(month, opts){
    ...
    Object.keys(byStudent).forEach(function(sid){
      ...
      DB.fees.push({ id: "FEE" + String(++seq).padStart(5, "0"), source: "batch", month: month,
        ...
        teacherId: b ? b.teacherId : "", feeType: "monthly", rate: full,
        gross: fee, discount: Math.max(0, full - fee), waived: false, locked: false,
        dueDate: due, remarks: remark, createdAt: new Date().toISOString() });
      created++;
    });
    persist();
  ...
  buildIndMonth(month){
    ...
    DataService.getPlans().forEach(function(pl){
      ...
      const gross = Logic.computeGross(pl.feeType, pl.rate, stats);
      ...
      if (found) {
        ...
        if (found.locked || found.reconciled || (prepaid && Logic.paidOn(found.id) > 0)) { skipped++; return; }
        if (round2(found.gross) !== gross) {
  ...
  reconcileMonth(month){
    ...
    DataService.getFees({ month: month, source: "individual" }).forEach(function(f){
      ...
      const actualGross = Logic.computeGross(f.feeType, f.rate, actual);
...
Object.assign(Logic, {
  payrollView(t, month){
    ...
    if (t.indRateType === "perClass") { ind = cs.completed * rate; indBasis = cs.completed + " classes × " + money(rate); }
    else if (t.indRateType === "perDay") { ind = cs.days * rate; indBasis = cs.days + " days × " + money(rate); }
    else if (t.indRateType === "perHour") { ind = (cs.minutes / 60) * rate; indBasis = round2(cs.minutes / 60) + " hours × " + money(rate); }
    else if (t.indRateType === "monthly") { ind = rate; indBasis = "Monthly fixed"; }
    else if (t.indRateType === "percent") {
      const base = Logic.feeViews({ month: month, source: "individual", teacherId: t.id }).reduce((s, v) => s + v.netFee, 0);
