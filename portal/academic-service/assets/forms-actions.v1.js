"use strict";
/* Academic Service — forms-actions.v1.js (split from index.html). Immutable: edit a copy as v2. */
/* ==========================================================================
   FORMS
   ========================================================================== */
...
function ccOptions(sel){
  const list = [["91","🇮🇳 +91"],["971","🇦🇪 +971"],["966","🇸🇦 +966"],["974","🇶🇦 +974"],["965","🇰🇼 +965"],
    ...
    ["61","🇦🇺 +61"],["49","🇩🇪 +49"],["33","🇫🇷 +33"],["92","🇵🇰 +92"],["880","🇧🇩 +880"],["94","🇱🇰 +94"]];
...
function studentForm(s, isNew){
  ...
  return '<div class="form-grid">' +
    ...
    field("WhatsApp number", '<div class="ccrow"><select class="input ccsel" name="whatsappCC">' + ccOptions(s.whatsappCC || Settings().countryCode || "91") + '</select>' +
      '<input class="input mono" name="whatsapp" value="' + esc(s.whatsapp || "") + '" placeholder="Number without country code" style="flex:1"></div>') +
...
function planForm(p, presetStudent){
  ...
  return '<div class="form-grid">' +
    ...
    '<div class="field span2"><label>Fee method</label>' +
      sel("feeType", optList(FEE_METHODS.map(m => ({ value: m.value, label: m.label + " — " + m.hint })), p.feeType || set.defaultFeeMethod)) +
      '<span class="hint">The monthly fee is worked out from the class log using this method. Nothing is typed in.</span></div>' +
    field("Rate", '<input class="input" type="number" name="rate" min="0" step="0.01" value="' + esc(p.rate || 0) + '">') +
...
function teacherForm(t){
  ...
  return '<div class="form-grid">' +
    ...
    field("WhatsApp", '<div class="ccrow"><select class="input ccsel" name="whatsappCC">' + ccOptions(t.whatsappCC || Settings().countryCode || "91") + '</select>' +
      '<input class="input mono" name="whatsapp" value="' + esc(t.whatsapp || "") + '" placeholder="Number only" style="flex:1"></div>') +
    ...
    field("Individual class pay", sel("indRateType", optList(IND_RATE_TYPES, t.indRateType || "perClass"))) +
    field("Individual rate", '<input class="input" type="number" name="indRate" min="0" step="0.01" value="' + esc(t.indRate || 0) + '">') +
...
const Actions = {
  ...
  "enrol-new": (batchId) => openModal({
    ...
    onSubmit: function(d){
      ...
      const id = DataService.saveEnrollment({ studentId: d.studentId, batchId: d.batchId, subClassId: d.subClassId || "",
        joiningDate: d.date, leavingDate: "", monthlyFee: monthlyFee,
      ...
      if (d.raiseFirst && !(holdB && holdB.waiveFee)) {
        ...
        DataService.saveFee({ source: "batch", month: m, studentId: d.studentId, batchId: d.batchId,
          ...
          feeType: "monthly", rate: monthlyFee, gross: monthlyFee, discount: 0, waived: false, locked: false,
          dueDate: due, remarks: "First month (prepaid)", createdAt: new Date().toISOString() });
      }
  ...
  "plan-new": (studentId) => openModal({ title: "New class plan", wide: true, body: planForm({}, studentId), submitText: "Save plan",
    onSubmit: function(d, form){
      ...
      ["duration","rate","discount","dueDay"].forEach(k => d[k] = +d[k] || 0);
      ...
      if (raiseFirst && (+d.rate || 0) > 0 && !(holdI && holdI.waiveFee)) {
        const gross = Math.max(0, (+d.rate || 0) - (+d.discount || 0));
        DataService.saveFee({ source: "individual", month: m, studentId: d.studentId, planId: id,
          teacherId: d.teacherId || "", feeType: "monthly", rate: +d.rate || 0, gross: gross, discount: +d.discount || 0,
  ...
  "fee-edit": (id) => {
    ...
    openModal({ title: "Adjust fee — " + v.studentName,
      body: '<div class="note">' + streamChip(v.source) + ' ' + esc(monthLabel(v.month)) + ' · gross comes from ' +
        ...
        '. Payments already received (' + money(v.paidAmount) + ') are untouched. A discount is not an expense — it lowers the fee.</div>' +
        '<div class="form-grid">' +
      ...
      onSubmit: function(d){
        DataService.saveFee({ id: id, gross: +d.gross || 0, discount: +d.discount || 0, dueDate: d.dueDate,
          waived: !!d.waived, locked: !!d.locked, remarks: d.remarks });
        toast("Fee record updated", "ok"); render();
      } });
  },
  "fees-csv": (id, el) => {
...
Object.assign(Actions, {
  ...
  "staff-new": () => openModal({ title: "Add staff", wide: true, body: teacherForm({ kind: "staff" }), submitText: "Save staff",
    onSubmit: function(d){
      ...
      d.indRate = +d.indRate || 0; d.batchRate = +d.batchRate || 0; d.monthlySalary = +d.monthlySalary || 0; d.kind = "staff"; delete d.id; joinCC(d);
  ...
  "teacher-new": () => openModal({ title: "Add teacher", wide: true, body: teacherForm({}), submitText: "Save teacher",
    onSubmit: function(d){
      if (!d.name) { toast("Name is needed", "bad"); return false; }
      d.indRate = +d.indRate || 0; d.batchRate = +d.batchRate || 0; d.monthlySalary = +d.monthlySalary || 0; delete d.id; joinCC(d);
      const id = DataService.saveTeacher(d);
  ...
  "teacher-edit": (id) => openModal({ title: "Edit " + id, wide: true, body: teacherForm(DataService.getTeacher(id)), submitText: "Save changes",
    onSubmit: function(d){ d.id = id; d.indRate = +d.indRate || 0; d.batchRate = +d.batchRate || 0; d.monthlySalary = +d.monthlySalary || 0; joinCC(d);
